#!/usr/bin/env python3
# orrery_pro_v9_final.py
# Versão final com correção definitiva para o problema de fuso horário UTC em todos os sistemas.

import math
import sys
from datetime import datetime, timedelta, tzinfo, timezone

# A importação de 'timezone' do 'datetime' é a chave para a correção.
        ...
        try:
            ...
            for name, body in bodies_to_check.items():
                ...
                while t.ut < end_time.ut:
                    ...
                    next_transit_event = SearchHourAngle(body, self.observer, 0.0, t, 1)
                    ...
                    next_event_time, next_name, next_type = found_events_in_step[0]
                    
                    is_duplicate = any(abs(next_event_time.ut - ev[0].ut) < 1e-4 and next_name == ev[1] for ev in events)
                    
                    if not is_duplicate:
                        events.append(found_events_in_step[0])
                    
                    t = next_event_time.AddDays(1.0 / (24 * 60))
                    if t.ut >= end_time.ut: break
...
if __name__ == '__main__':
    if not hasattr(Time, 'from_datetime'):
        def from_datetime(dt):
            ...
            # CORREÇÃO DEFINITIVA: Usa timezone.utc, que é nativo e sempre disponível.
